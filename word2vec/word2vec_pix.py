...
class Word2Vec(utils.SaveLoad):
    ...
    def build_vocab_pix(self, ion_df_path, ds_dir, min_count=None, sample=None, fdr=None, int_per=None, pix_per=None, quan=None):
        ...
        ion_freq = dict.fromkeys(ion_df.index,0)
        for f in os.listdir(ds_dir):
            ...
            freq_counts = dict(filt_df['ion_ind'].value_counts())
        ...
        ion_freq_res = {i: f for i,f in ion_freq.items() if f>= min_count}
        retain_total = sum(itervalues(ion_freq_res))

        #self.total_words = sum(itervalues(ion_freq_res))
        ...
        for i in ion_freq_res:
            v = ion_freq_res[i]
            #word = ion_df.get_value(i, 'formula')+ion_df.get_value(i, 'adduct')
            word = str(i) # intead of ion name we just keep it's pandas id to make processing faster, it has to be a string
        ...
        logger.info("collected %i word types from a corpus of %i total examples", len(ion_freq_res), self.corpus_count)
...
class PixelCorpus(object):
    ...
    def __iter__(self):
        for f in os.listdir(self.ds_dir):
            ...
            #int_thresh = ds_df['int'].max() * self.i
            ...
            for _, c_row in sampled_coord_df.iterrows():
                ...
                #ion_ids = window_rows.ion_ind.unique()
                #yield ion_ids

                exp_ion_ids = []
                ion_counts = dict(window_rows['ion_ind'].value_counts())
                for ion in ion_counts:
                    for i in range(0,ion_counts[ion]): exp_ion_ids.append(ion)
                yield exp_ion_ids
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ion", help="Ion data msgpack", required=True)
    ...
    #ion_freq = {}
    #for w_list in corpus:
    #    if 33125 in w_list:
    #        for e in w_list:
    #            if e not in ion_freq: ion_freq[e] = 0
    #            ion_freq[e] += 1

    #import operator
    #sorted_ion_freq = sorted(ion_freq.items(), key=operator.itemgetter(1),reverse=True)

    #for (i,f) in sorted_ion_freq:
    #    print(ion_df.get_value(i, 'formula')+ion_df.get_value(i, 'adduct'),f)
    #exit(0)
